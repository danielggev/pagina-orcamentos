...
const Carrinho = (() => {
  ...
  function renderDrawer() {
    ...
    body.innerHTML = itens.map(item => `
      <div class="drawer-item" data-id="${item.id}">
        ...
        <div class="drawer-item__info">
          ...
          <div class="drawer-item__price-row">
            ${item.preco != null
              ? `<p class="drawer-item__preco">${item.preco.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>`
              : ''}
            ${item.superRapido || item.colecao === 'Super Rápido' ? `
              <span class="drawer-item__sr-flag drawer-item__sr-flag--inline">
                <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><polygon points="386.415,193.208 287.481,193.208 359.434,0 161.566,0 125.585,280.151 206.528,280.151 170.557,512"/></svg>
                Super Rápido
              </span>
            ` : ''}
          </div>
    ...
    const totalStr = totalGeral > 0
      ...
      : null;
    const abaixoMinimo = totalGeral > 0 && totalGeral < MINIMO_PEDIDO;
    ...
    footer.innerHTML = `
      ...
      ${abaixoMinimo ? `<p class="drawer-aviso-minimo">Mínimo de R$ 60,00 por pedido</p>` : ''}
    ...
    document.getElementById('drawer-btn-wpp').addEventListener('click', () => {
      const checkoutHref = isSubpage ? '../checkout.html' : 'checkout.html';
  ...
  const MINIMO_PEDIDO = 60;

  function totalPedido() {
    return getItens().reduce((acc, i) => acc + (i.preco || 0), 0);
  }
  ...
  function enviarWhatsApp() {
    ...
    if (total > 0 && total < MINIMO_PEDIDO) {
      alert(`Valor mínimo de pedido: R$ ${MINIMO_PEDIDO.toFixed(2).replace('.', ',')}.\nSeu pedido atual está em ${total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.\nAdicione mais produtos para continuar.`);
      return;
    }
